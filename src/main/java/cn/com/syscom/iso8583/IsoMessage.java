...
public class IsoMessage implements Serializable {
	 ...
	 * 
	 */
//	private static final long serialVersionUID = -6995143945148818039L;
	...
	private boolean isTimeOutTrans = false;
    private long beginVisaTime = 0;
    private long endVisaTime = 0;
    private long beginEsbTime = 0;
    private long endEsbTime = 0;
    
    private byte[] networkMatch = new byte[0];
	...
	public boolean isTimeOutTrans() {
		return isTimeOutTrans;
	}


	public void setTimeOutTrans(boolean isTimeOutTrans) {
		this.isTimeOutTrans = isTimeOutTrans;
	}
	
    public long getBeginVisaTime() {
		return beginVisaTime;
	}

	public void setBeginVisaTime(long beginTime) {
		this.beginVisaTime = beginTime;
	}
	
    public long getEndVisaTime() {
		return endVisaTime;
	}

	public void setEndVisaTime(long endTime) {
		this.endVisaTime = endTime;
	}
	
    public long getBeginEsbTime() {
		return beginEsbTime;
	}

	public void setBeginESBTime(long beginTime) {
		this.beginEsbTime = beginTime;
	}
	
    public long getEndEsbTime() {
		return endEsbTime;
	}

	public void setEndEsbTime(long endTime) {
		this.endEsbTime = endTime;
	}
	
	public byte[] getNetworkMatch() {
		return networkMatch;
	}

	public void setNetworkMatch(byte[] networkMatch) {
...
/*    		if (lengthBytes == 4) {
    ...
    public byte[] writeData() {
    	...
    	for (int i = 2; i < 129; i++) {
    		...
    		if (v != null) {
        		try {
        			v.write(bout, binary, forceStringEncoding,encoding);
        		} catch (IOException ex) {
        			//should never happen, writing to a ByteArrayOutputStream
        		}
    		}
    ...
    protected BitSet createCustomBitmapBitSet() {
    ...
    protected BitSet createCustom63BitmapBitSet() {
    ...
    /** lvbang 含有子bitmap的域调用. */
    public byte[] customField63WriteData() {
    	ByteArrayOutputStream bout = new ByteArrayOutputStream();

    	//Bitmap
        BitSet bs = createCustom63BitmapBitSet();
    	//Write bitmap to stream
    	if (binary || binBitmap) {
    		int pos = 128;
    		int b = 0;
    ...
    public byte[] customFieldWriteData() {
    	ByteArrayOutputStream bout = new ByteArrayOutputStream();

    	//Bitmap
        BitSet bs = createCustomBitmapBitSet();
