...
import cn.com.syscom.iso8583.parse.DateTimeParseInfo;
 ...
 */
public class MessageFactory<T extends IsoMessage> implements Serializable {
	...
	/**Field37**/
	private boolean setRetrievalReferenceNumber;
	private int etx = -1;
	...
	public T newMessage(int type) {
		...
		if (setRetrievalReferenceNumber) {
			 Calendar c=new GregorianCalendar();
				String str = new String();
				str = Integer.toString(c.get(Calendar.YEAR)).substring(3) + String.format("%03d", c.get(Calendar.DAY_OF_YEAR)) + String.format("%02d", c.get(Calendar.HOUR_OF_DAY));
				if (traceGen != null) {
				str =  str + m.getAt(11).toString();

			}else {
				str =  str + new IsoValue<>(IsoType.NUMERIC, traceGen.getLastTrace(), 6).toString();
			}
			m.setValue(37, str, IsoType.ALPHA, 12);
		}
		return m;
	}
						...
						if (val != null) {
							if (val.getType() == IsoType.NUMERIC || val.getType() == IsoType.DATE10
								...
								pos += (val.getLength() / 2) + (val.getLength() % 2);
								 ...
								 pos = pos + (buf[pos] & 0xff);//lvbang
								//pos += val.getLength();
                            }else if (val.getType() == IsoType.LLNUM
                                    || val.getType() == IsoType.LLLNUM) {
								 pos = pos + ((buf[pos] & 0xff) / 2) + ((buf[pos] & 0xff) % 2);//lvbang
								//pos += val.getLength();
                            }else {
								pos += val.getLength();
							}
			...
			for (Integer i : index) {
				...
				if (bs.get(i - 1)) {
					if (ignoreLast && pos >= buf.length && i.intValue() == index.get(index.size() -1)) {
						...
						if (val != null) {
							if (val.getType() == IsoType.LLVAR || val.getType() == IsoType.LLNUM || val.getType() == IsoType.LLBIN) {
								 pos = pos + ((buf[pos] - 48) * 10) + (buf[pos + 1] - 48);//lvbang
								//pos += val.getLength();
                            }else if (val.getType() == IsoType.LLLVAR || val.getType() == IsoType.LLLNUM) {
								 pos = pos + ((buf[pos] - 48) * 100) + ((buf[pos + 1] - 48) * 10)
				                            + (buf[pos + 2] - 48);//lvbang
								//pos += val.getLength();
                            }else if (val.getType() == IsoType.LLLLVAR) {
								 pos = pos + ((buf[pos] - 48) * 1000) + ((buf[pos + 1] - 48) * 100)
				                            + ((buf[pos + 2] - 48) * 10) + (buf[pos + 3] - 48);//lvbang
								//pos += val.getLength();
                            }else if (val.getType() == IsoType.LLLBIN) {
                       		 pos = pos + ((buf[pos] - 48) * 100) + ((buf[pos + 1] - 48) * 10)
			                            + (buf[pos + 2] - 48);//lvbang
                           }else {
	                        	   if (	 val.getType() == IsoType.BINARY
	                        			|| val.getType() == IsoType.LLLBIN
										|| val.getType() == IsoType.LLBIN
										|| val.getType() == IsoType.LLLLBIN) {
	                        		   pos += val.getLength();
						...
						if (val.getType() == IsoType.LLVAR || val.getType() == IsoType.LLBIN || val.getType() == IsoType.LLNUM) {
							pos += 2;
						} else if (val.getType() == IsoType.LLLVAR || val.getType() == IsoType.LLLBIN || val.getType() == IsoType.LLLNUM) {
							pos += 3;
	...
	public boolean isSetRetrievalReferenceNumber() {
		return setRetrievalReferenceNumber;
	}
	public void setSetRetrievalReferenceNumber(boolean setRetrievalReferenceNumber) {
		this.setRetrievalReferenceNumber = setRetrievalReferenceNumber;
	}
	...
	public Map<Integer, List<Integer>> getParseOrder() {
		return parseOrder;
	}


public T parseCustomField(byte[] buf, int isoHeaderLength, boolean binaryIsoHeader)
    	throws ParseException, UnsupportedEncodingException {
	final int minlength = isoHeaderLength+(useBinary?2:4)+(binBitmap||useBinary ? 8:16);
	if (buf.length < minlength) {
		throw new ParseException("Insufficient buffer length, needs to be at least " + minlength, 0);
	...
	int pos = 0;
	if (useBinary || binBitmap) {
        final int bitmapStart = isoHeaderLength + (useBinary ? 2 : 2);
		for (int i = bitmapStart; i < 8+bitmapStart; i++) {
			int bit = 128;
			for (int b = 0; b < 8; b++) {
				bs.set(pos++, (buf[i] & bit) != 0);
...
public T parseCustomField63(byte[] buf, int isoHeaderLength, boolean binaryIsoHeader)
    	throws ParseException, UnsupportedEncodingException {
	final int minlength = isoHeaderLength+(useBinary?2:4)+(binBitmap||useBinary ? 3:16);
	if (buf.length < minlength) {
		throw new ParseException("Insufficient buffer length, needs to be at least " + minlength, 0);
	}
	final T m;
...
/*    } else if (forceStringEncoding) {
	...
	m.setType(type);
	//Parse the bitmap (primary first)
	final BitSet bs = new BitSet(24);
	int pos = 0;
	if (useBinary || binBitmap) {
        final int bitmapStart = isoHeaderLength + (useBinary ? 2 : 2);
		for (int i = bitmapStart; i < 3+bitmapStart; i++) {
			int bit = 128;
			for (int b = 0; b < 8; b++) {
				bs.set(pos++, (buf[i] & bit) != 0);
				bit >>= 1;
