...
public class Example {
	...
	public static void main(String[] args) throws Exception {
		MessageFactory mfact = ConfigParser.createFromClasspathConfig("j8583_config.xml");
		mfact.setAssignDate(true);
		mfact.setTraceNumberGenerator(new SimpleTraceGenerator((int)(System.currentTimeMillis() % 100000)));
		LineNumberReader reader = new LineNumberReader(new InputStreamReader(Example.class.getClassLoader().getResourceAsStream("parse.txt")));
		...
		FileOutputStream fout = new FileOutputStream("iso.bin");
		m.write(fout, 2);
		fout.close();
		print(m);
		System.out.println("PARSE BINARY FROM FILE");
		byte[] buf = new byte[2];
		FileInputStream fin = new FileInputStream("iso.bin");
